...
if [ -d /mnt/disks/platform/usr/local/sbin ]; then
  ...
  #JEKYLL_GITHUB_TOKEN=${GITHUB_ACCESS_TOKEN} DISABLE_WHITELIST=true jekyll build --profile -t -p /home/runner/_site/_plugins -d ${GITHUB_WORKSPACE}/_site
  #git config --global --add safe.directory ${GITHUB_WORKSPACE}/site && cd ${GITHUB_WORKSPACE}/_site && touch .nojekyll
